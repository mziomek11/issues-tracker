...
class ChangeIssueTypeCommandHandlerTest extends IssueCommandHandlerTest {
    @ParameterizedTest
    ...
    void eventSourcingHandlerIsNotCalledWhenServiceThrows(RuntimeException gatewayException) {
        ...
        var sut = new ChangeIssueTypeCommandHandler(eventSourcingHandler, organizationGateway);

        // Assert
        assertThatExceptionOfType(gatewayException.getClass()).isThrownBy(() -> sut.handle(CHANGE_ISSUE_TYPE_COMMAND));
    ...
    void eventSourcingHandlerIsNotCalledWhenIssueDoesNotExist() {
        ...
        var sut = new ChangeIssueTypeCommandHandler(eventSourcingHandler, organizationService);

        // Assert
        assertThatExceptionOfType(IssueNotFoundException.class).isThrownBy(() -> sut.handle(CHANGE_ISSUE_TYPE_COMMAND));
        verify(eventSourcingHandler, times(0)).save(any());
    }
}
