...
class RenameIssueCommandHandlerTest extends IssueCommandHandlerTest {
    @ParameterizedTest
    ...
    void eventSourcingHandlerIsNotCalledWhenServiceThrows(RuntimeException gatewayException) {
        ...
        var sut = new RenameIssueCommandHandler(eventSourcingHandler, organizationGateway);

        // Assert
        assertThatExceptionOfType(gatewayException.getClass()).isThrownBy(() -> sut.handle(RENAME_ISSUE_COMMAND));
    ...
    void eventSourcingHandlerIsNotCalledWhenIssueDoesNotExist() {
        ...
        var sut = new RenameIssueCommandHandler(eventSourcingHandler, organizationService);

        // Assert
        assertThatExceptionOfType(IssueNotFoundException.class).isThrownBy(() -> sut.handle(RENAME_ISSUE_COMMAND));
        verify(eventSourcingHandler, times(0)).save(any());
    }
}
