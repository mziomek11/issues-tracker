...
class HideIssueCommentCommandHandlerTest extends IssueCommandHandlerTest {
    @ParameterizedTest
    ...
    void eventSourcingHandlerIsNotCalledWhenServiceThrows(RuntimeException gatewayException) {
        ...
        var sut = new HideIssueCommentCommandHandler(eventSourcingHandler, organizationGateway);

        // Assert
        assertThatExceptionOfType(gatewayException.getClass()).isThrownBy(() -> sut.handle(HIDE_ISSUE_COMMENT_COMMAND));
    ...
    void eventSourcingHandlerIsNotCalledWhenIssueDoesNotExist() {
        ...
        var sut = new HideIssueCommentCommandHandler(eventSourcingHandler, organizationService);

        // Assert
        assertThatExceptionOfType(IssueNotFoundException.class).isThrownBy(() -> sut.handle(HIDE_ISSUE_COMMENT_COMMAND));
        verify(eventSourcingHandler, times(0)).save(any());
    }
}